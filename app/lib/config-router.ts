...
// ─── WhatsApp 渠道管理 ───
// 参考 openclaw 项目 src/channels/plugins/onboarding/whatsapp.ts 的完整流程：
// QR 链接 → 手机模式选择 → DM 策略配置 → 白名单号码 → 保存
...
// 特别处理 515 状态码：配对已成功但 WhatsApp 要求重启连接，凭据已保存
configRouter.post('/whatsapp/link/poll', async (c) => {
  ...
  try {
    ...
    // 515 = 配对成功，凭据已保存，但 WhatsApp 要求重启 socket
    if (!connected && /515|restart required/i.test(message)) {
      ...
      // 最后手段：检查凭据文件是否已存在（515 发生在配对之后，凭据已保存）
      if (checkCredsExist()) {
...
// WhatsApp DM 策略配置（对应 openclaw 的 promptWhatsAppAllowFrom 流程）
// 接受: { phoneMode, phoneNumber?, dmPolicy?, allowFrom? }
configRouter.post('/whatsapp/configure', async (c) => {
  try {
    const { phoneMode, phoneNumber, dmPolicy, allowFrom } = await c.req.json();

    if (phoneMode === 'personal') {
      // ── 个人手机模式（对应 openclaw phoneMode === 'personal'）──
      // 设置: selfChatMode=true, dmPolicy=allowlist, allowFrom=[号码]
